...
async def upload_image(file: UploadedImage, db: DatabaseDependency, storage: StorageDependency):
    """
    ...
    if file_size > max_file_size_bytes:
        raise HTTPException(
            status_code=400, detail=f"File size ({file_size // 1024}KB) exceeds maximum allowed ({max_file_size_kb}KB)"
        )
    ...
    await db.create_image(
        image_id=image_id, storage_url=storage_url, width=width, height=height, file_size=file_size, hash=file_hash
    )

    # Create pending job
    job_id = await db.create_job(image_id)

    return UploadResponse(job_id=job_id, image_id=image_id, status="queued")


@router.get("/images/{image_id}", response_model=ImageResponse)
async def get_image(image_id: str, db: DatabaseDependency, storage: StorageDependency):
    """
    ...
    for det in image.get("detections", []):
        detections.append(
            DetectionSummary(
                ...
                bbox=BBox(x=det["bbox_x"], y=det["bbox_y"], w=det["bbox_w"], h=det["bbox_h"]),
            )
        )
...
async def get_detection(detection_id: str, db: DatabaseDependency):
    """
    ...
    if detection.get("polygon") and detection["polygon"].get("points_json"):
        try:
            ...
            polygon_data = PolygonData(points=points_list, simplified=detection["polygon"].get("simplified", False))
        except Exception:
            pass
    ...
    return DetectionDetail(
        ...
        bbox=BBox(x=detection["bbox_x"], y=detection["bbox_y"], w=detection["bbox_w"], h=detection["bbox_h"]),
...
async def get_job_status(job_id: str, db: DatabaseDependency):
    """
